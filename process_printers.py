...
if new_entries:
    try:
        ...
        # batch 寫入消耗紀錄
        # 對 type='consume' / 'aborted' 用 print_guid 當 doc_id，自動防重複（同 guid 寫多次只會覆蓋）
        # 其他類型（stockin / manual）仍用自動 id（不適用 print_guid）
        BATCH_SIZE = 400
        for i in range(0, len(new_entries), BATCH_SIZE):
            batch = db.batch()
            for entry in new_entries[i:i+BATCH_SIZE]:
                guid = entry.get('print_guid', '')
                if guid and entry.get('type') in ('consume', 'aborted'):
                    # 用 print_guid 當 doc_id，重跑時自動覆蓋同 guid 紀錄
                    doc_ref = db.collection('inventory_history').document(guid)
                else:
