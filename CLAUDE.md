...
- ⚠ **手動同步跳「同步失敗：internal」＝Cloud Run 沒開公開叫用**（2026-09-18，帳號到期恢復後發生）：log 只有 `The request was not authenticated ... Empty Authorization header`，函式本體根本沒執行。解法是 Cloud Run → `sync-formlabs-manual` → 安全性 →「允許未經驗證的叫用」（程式內仍會檢查登入與 admin）。`sync-eiger-manual` 當時不受影響。這是 GCP 權限設定，由使用者在 Console 操作
...
- **責任工程師＝實際執行列印的人**（2026-09-11）：Formlabs 與 Markforged 都有，寫進 `inventory_history.operator`
  ...
  - **帳號名稱設錯的例外**（2026-09-15）：Formlabs 上 Jimmy 的帳號名稱設成 `2024092`。`functions/main.py` 的 `OPERATOR_ALIASES = {"2024092": "Jimmy"}`，`normalize_operator()` 在**寫進 Firestore 之前**就換掉（Formlabs 與 Markforged 都套），前端再依對照表顯示成「廖璟程 (Jimmy)」
    - ⚠ **改資料、不是只換顯示**：篩選、月度佔比、匯出讀的都是存進去的值，只在某個畫面換掉的話其他地方仍是錯的
    - 已經寫進去的舊值由 `_migrate_operator_aliases()` 一次性更正，完成後把對照表簽章寫進 `inventory/main.operator_alias_sig`，之後每輪比對相同就跳過（0 次讀取）。**改對照表簽章就會變，自動再跑一次**
    - 片段比對只切 `. _ @ -` 與空白（`jiliao.2024092` 會換、`12024092`／`jiliao2024092` 不會換），刻意保守
    - ⚠ 使用者確認這是**唯一**一個設定名稱時的錯誤，不要把它當成通用機制亂加
  - ⚠ **兩邊都只存名稱、不存 email 與 id**：消耗紀錄是全公司登入者都讀得到的 collection
  - **匯出併進既有的「責任工程師」欄（第 10 欄），不另開欄位**：先做成第 20 欄，2026-09-11 使用者決定統一用這個命名。⚠ **消耗紀錄有 `operator` 就直接用它，工單上的責任工程師只補空白**（2026-09-18 使用者決定，反轉了原本「工單優先」的規則）。join 在 `applyWorkboardJoin()`（同步函式，`tools/test_print_log_export.js` 有測）；業務仍以工單為準，但工單沒填業務時不可洗掉已帶入的值
...
- **工程測試掛自家公司**（`ENG_TEST_COMPANY`＝實威國際股份有限公司）：
  ...
  - **備註沒照格式**（對不到工作類別**且**沒有 APP 單號，含空備註）→ 業務與客戶名稱都帶入實威國際（2026-09-18 使用者決定）。有單號但類別對不到的**不算**，交給工單 join
...
- ⚠ **對照表裡的材料代碼不一定等於 API 實際回傳的代碼**（2026-09-03 事故，查了三輪才破案）：Elastic 50A 在表裡登記成 `FLFLES`（本專案自編），Formlabs 實際回傳的是 **`FLELCL`**（官方料號 `RS-F2-ELCL-01`）。入庫走下拉選單存 `FLFLES`、消耗從 API 進來是 `FLELCL`，兩個 key 對不起來 → `apply_stock_deductions` 自建一個 `total_ml=0` 的幽靈 key → 扣 0 → 全額進 `stock_shortfalls` → **每輪都跳「消耗紀錄可能有誤」，而庫存數字完全正常（根本沒被扣到）**。已用 `FAMILY_REMAP['FLELCL']='FLFLES'` 收斂（方向往既有 key 併，才不必搬資料）
  ...
  - ⚠ **`disabled_overrides` 優先於 `disabled_materials`，刪除材料時必須一起清**（2026-09-21 使用者回報中部 Flexible 80A V1.1）：只加黑名單的話，刪除當下看起來成功（`stock` 的 key 真的刪了），重新整理就又從 history／cartridges 被撈回清單，畫面上沒有任何線索。統一走 `markMaterialDisabled()`（比對**家族碼**，兩份清單存的形式可能一個是代碼一個是名稱），`tools/test_material_input.js` 有守
