...
# 也被拉成 FLFL8011。那之後任何 V2（FLFL8002）的列印都會被判成舊版 —— log 有一筆
# 「舊版本不扣庫存: 'FLFL8002'」，但那筆是中部 9/15 **中止**的列印，本來就不扣，
# 所以**中部實際上沒有漏扣**（使用者 2026-09-16 確認；其餘中部 V2 正常印完的都有扣）。
# 這一筆別名防的是「之後」中部正常印完的 V2 被誤判，不是修已經發生的漏扣。
...
def version_code_of(raw) -> Optional[str]:
    ...
    ★★ 為什麼需要：版本判斷原本只認得代碼（取末 2 碼）。但有些機台回傳的是**名稱**
       （2026-09-15 南部 Form4B 的 80A V1.1 被當成 V2，推定是回傳名稱），名稱解析不出版本號 → 依保守規則
